...
async def find_backend_language(Link):
    ...
    if "Server" in response.headers:
        ...
        print(f"{Fore.RESET}{Style.BRIGHT}**[INFO]Looks like the backend language of {Fore.RESET}{Fore.RED}{Link} is with {Fore.RESET}{Fore.GREEN}{server_header}.{Fore.RESET}Do you want to continue?(y/q)**")
    else:
...
async def Find_back_end_auto(Link):
    ...
    if "Server" in response.headers:
        server_header = response.headers["Server"]
        # print(Fore.GREEN+"[INFO] looks like the Backend language is with", Fore.RESET+Fore.RED+server_header)
        return f"{Fore.RESET}{Style.BRIGHT}**[INFO]Looks like the backend language of {Fore.RESET}{Fore.RED}{Link} is with {Fore.RESET}{Fore.GREEN}{server_header}.{Fore.RESET}Do you want to continue?(y/q)**"
    else:
