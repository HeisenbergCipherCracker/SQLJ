...
"""
Function: mysql_stored_procedure_injection
Description: Performs MySQL stored procedure injection attacks on a target website.
...
Note:
- The payloads for MySQL stored procedure injection attacks are sourced from https://github.com/payloadbox/sql-injection-payload-list.
...
2. Use the 'mysql_stored_procedure_injection(urls)' function to perform MySQL stored procedure injection attacks on the specified URLs.

Example:
```python
from sql_injection_exploit import mysql_stored_procedure_injection

urls_to_attack = ["http://example.com/login", "http://testsite.net/login"]
for url in urls_to_attack:
    asyncio.run(mysql_stored_procedure_injection(url))
"""
...
async def Procedure_Attack(urls):
...
# asyncio.run(Precudure_Attack("http://testfire.net/login.jsp"))
